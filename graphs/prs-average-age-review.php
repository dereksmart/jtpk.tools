...
$total_issues = $wpdb->get_results( 'SELECT * FROM gh_stats WHERE meta_key = "average_review_age_in_days" order by date ASC', OBJECT );

$max_y = 0;
foreach ( $total_issues as $iss ) {
	if ( strtotime( $iss->date ) < strtotime( '-40 days' ) ) {
		continue;
	}
...
$total_issues = $wpdb->get_results( 'SELECT * FROM gh_stats WHERE meta_key = "median_review_age_in_days" order by date ASC', OBJECT );
...
$graph->title->Set('Average "needs review" PR Age in Days (Mean and Median)');
